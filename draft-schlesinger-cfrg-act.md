...
The protocol uses the Ristretto group {{RISTRETTO}}, which provides a prime-order group abstraction over Curve25519. The key parameters are:
...
Note: The reference implementation currently uses version "v0.2.0" while this specification defines version "v1.0". Implementations MUST use the exact version string for the protocol version they implement. Different version strings create incompatible protocols, which is intentional to prevent version downgrade attacks and ensure implementations are explicitly compatible.
...
For debugging implementations, here are the expected intermediate values for the main test case (100 credits, spend 30):
...
These test vectors demonstrate:
...
5. Correct binary decomposition for range proofs

### Implementation Validation

Implementations can validate correctness by:
...
4. Checking that double-spends produce identical nullifiers

Note: Full cryptographic values (points, signatures) depend on the specific implementation's representation and cannot be compared directly across implementations.
